...
export class MikroOrmTransferRepository implements ITransferRepository {
	...
	async save(transfer: Transfer): Promise<void> {
		const originAccountRef = this.em.getReference(AccountEntity, transfer.originId.value)
		const targetAccountRef = this.em.getReference(AccountEntity, transfer.targetId.value)
