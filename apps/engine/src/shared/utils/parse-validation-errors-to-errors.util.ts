...
export function parseValidationErrorsToErrorsUtil(validationErrors: ValidationError[]) {
	const validationErrorsGroupedByProperty = validationErrors.reduce<Record<string, ValidationError[]>>(
		(previousValue, error) => {
	...
	const violationsGroupedByProperty = Object.entries(validationErrorsGroupedByProperty).reduce<
		Record<string, Violation[]>
	>((previousValue, [currentProperty, currentErrors]) => {
		const constraints = currentErrors.filter((error) => !!error.constraints).map((error) => error.constraints)

		const violations = Object.keys(constraints).map((error) => fromValidationErrorToViolationUtil(error))
